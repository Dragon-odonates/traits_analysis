...
for(i in 1:5) {
  datlist[[i]] <- read_excel(here(file.path("data", 
                                            "OdonDatabase_life_hist_traits.xlsx")),
                               sheet = i)
}
names(datlist) <- c("taxonomy", "trend", "thermal", "imago", "exuviae")
```
...
datlist$imago$Species_name <- str_remove_all(datlist$imago$Species_name, 
                                             pattern = "\\*")
datlist$exuviae$Species_name <- str_remove_all(datlist$exuviae$Species_name, 
                                               pattern = "\\*")
...
# # To check
# df <- df |> 
#   mutate(Exuvia_Body_length = ifelse(Exuvia_Body_length == "Not relevant", 
#                                      Exuvia_Tot_length, Exuvia_Body_length))
# df <- df |> 
#   mutate(Exuvia_Caudal_lamellae_length = ifelse(Exuvia_Caudal_lamellae_length == "Not relevant", 
#                                      Exuvia_Tot_length, Exuvia_Caudal_lamellae_length))
```

```{r}
# column types
# df$Body_size_Min <- as.numeric(df$Body_size_Min)
# df$Abdomen_length_Min <- as.numeric(df$Abdomen_length_Min)
...
df_relevant <- df_relevant |> 
  select(body_size, 
         ...
         Coloration_M, # Could be redefined as a gradient (light -> dark)
         ends_with("voltine"),
         Perchers_and_fliers,
         Overwintering_stage,
         Larval_strategy, 
         Oviposition_substrate) # add average latitude + sd
...
```{r}
levels(df_mvar$Coloration_M)

# recode coloration
darkness <- as.character(df_mvar$Coloration_M)

# Correct Y/y
darkness[darkness == "yellow/black"] <- "Yellow/black"
unique(darkness)

# Ordered colorkey
colorkey <- c("Dark/Black", "Azure blue/black", "Metallic green/blue", "Yellow/black", "Brownish", "Green/Yellow", "Gray-blue", "Red")

# Recode color
darkness <- match(darkness, colorkey)
df_mvar$Coloration_M <- darkness
```
...
```{r}
s.corcircle(mvar$c1/max(mvar$c1$CS1))

s.label(mvar$li)
s.label(mvar$li, labels = rownames(df_mvar))
```
...
s.corcircle(discri$fa*0.5)
s.corcircle(discri$gc*0.7, 
            plabels.col = "darkred", 
